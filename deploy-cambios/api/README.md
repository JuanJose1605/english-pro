...
5. **Carpeta de imágenes**
   - El editor sube las imágenes a `api/uploads/`. Esa carpeta ya viaja en el
     `dist/` con su `.htaccess`. Asegúrate de que tenga **permisos de escritura**
     (cPanel → *Administrador de archivos* → carpeta `uploads` → *Permisos* `755`).
   - Si tu hosting es estricto y PHP no puede crearla, créala tú manualmente.

6. **Entrar al panel**
   - Ve a `https://TU-DOMINIO/#/admin` e inicia sesión.

> **¿Ya tenías el blog instalado de antes?** Solo necesitas añadir la columna de
> la imagen de portada: en *phpMyAdmin* → pestaña *SQL*, ejecuta:
> `ALTER TABLE posts ADD COLUMN cover_image VARCHAR(255) DEFAULT NULL AFTER cover_to;`
...
| POST | `upload.php` | admin | Subir imagen (multipart, campo `file`) → `{ url }` |
